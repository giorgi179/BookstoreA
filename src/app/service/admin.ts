...
export class AdminService {
  ...
  updateBook(id: number, book: NewBook) {
    return this.http
      .put<Book>(`${this.API}/books/${id}`, book, { headers: this.#h() })
      .pipe(
  ...
  loadSubscribers() {
    return this.http
      .get<AppUser[]>(`${this.API}/users/subscribers`, { headers: this.#h() })
      .pipe(tap({ next: (res) => this.subscribers.set(res) }));
  }
  ...
  unverifyUser(id: number) {
    return this.http.patch(`${this.API}/users/${id}/unverify`, {}, { headers: this.#h() }).pipe(
      tap({
        next: () =>
          this.users.update((u) => u.map((x) => (x.id === id ? { ...x, isVerified: false } : x))),
      }),
    );
  }
  ...
  deleteUserCard(id: number) {
    return this.http.delete(`${this.API}/users/${id}/card`, { headers: this.#h() }).pipe(
      tap({
        next: () =>
          this.users.update((u) =>
            u.map((x) => (x.id === id ? { ...x, savedCardMasked: null } : x)),
          ),
      }),
    );
  }
  ...
  loadBaskets() {
    return this.http
      .get<BasketGroup[]>(`${this.API}/baskets`, { headers: this.#h() })
      .pipe(tap({ next: (res) => this.baskets.set(res) }));
  }

  deleteBasket(id: number) {
    return this.http
      .delete(`${this.API}/baskets/${id}`, { headers: this.#h() })
      .pipe(tap({ next: () => this.baskets.update((b) => b.filter((x) => x.id !== id)) }));
  }
      ...
      tap({
        next: () =>
          this.baskets.update((bs) =>
            bs.map((b) =>
              b.id === basketId ? { ...b, items: b.items.filter((i) => i.id !== itemId) } : b,
            ),
          ),
      }),
    );
  ...
  updateBasketItem(itemId: number, basketId: number, quantity: number) {
    return this.http
      .put(`${this.API}/basket-items/${itemId}`, quantity, { headers: this.#h() })
      .pipe(
        tap({
          next: () =>
            this.baskets.update((bs) =>
  ...
  loadPayments() {
    return this.http
      .get<Payment[]>(`${this.API}/payments`, { headers: this.#h() })
      .pipe(tap({ next: (res) => this.payments.set(res) }));
  }

  deletePayment(id: number) {
    return this.http
      .delete(`${this.API}/payments/${id}`, { headers: this.#h() })
      .pipe(tap({ next: () => this.payments.update((p) => p.filter((x) => x.id !== id)) }));
  }

  updatePaymentStatus(id: number, status: string) {
    return this.http
      .patch(`${this.API}/payments/${id}/status`, JSON.stringify(status), {
        headers: this.#h().set('Content-Type', 'application/json'),
      })
      .pipe(
        tap({
          next: () =>
            this.payments.update((p) => p.map((x) => (x.id === id ? { ...x, status } : x))),
        }),
      );
  }
  ...
  loadMessages() {
    return this.http
      .get<Message[]>(`${this.API}/messages`, { headers: this.#h() })
      .pipe(tap({ next: (res) => this.messages.set(res) }));
  }
  ...
  loadCategories() {
    return this.http
      .get<any[]>(`https://bookapi-oc2p.onrender.com/api/Category/get-all`, { headers: this.#h() })
      .pipe(tap({ next: (res) => this.categories.set(res) }));
  }

  addCategory(name: string) {
    return this.http
      .post<any>(`https://bookapi-oc2p.onrender.com/api/Category/add`, JSON.stringify(name), {
        headers: this.#h().set('Content-Type', 'application/json'),
      })
      .pipe(tap({ next: (res) => this.categories.update((c) => [...c, res.category]) }));
  }

  updateCategory(id: number, name: string) {
    return this.http
      .put<any>(
        `https://bookapi-oc2p.onrender.com/api/Category/update/${id}`,
        JSON.stringify(name),
        {
          headers: this.#h().set('Content-Type', 'application/json'),
        },
      ...
      .pipe(
        tap({
          next: (res) =>
            this.categories.update((c) => c.map((x) => (x.id === id ? res.category : x))),
        }),
      );
  }
  ...
  deleteCategory(id: number) {
    return this.http
      .delete(`https://bookapi-oc2p.onrender.com/api/Category/delete/${id}`, { headers: this.#h() })
      .pipe(tap({ next: () => this.categories.update((c) => c.filter((x) => x.id !== id)) }));
  }
